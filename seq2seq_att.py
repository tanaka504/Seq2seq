...
class LSTM_Decoder(Chain):
    def __init__(self, vocab_size, embed_size, hidden_size):
        super(LSTM_Decoder, self).__init__(
            ...
            eh = links.Linear(embed_size, 4 * hidden_size),
            hh = links.Linear(hidden_size, 4 * hidden_size),
            wc = links.Linear(hidden_size, hidden_size),
            wh = links.Linear(hidden_size, hidden_size),
            fy = links.Linear(hidden_size, vocab_size)
        )

    def __call__(self, y, c, h, hs_enc):
        e = functions.tanh(self.ye(y))
        c, h = functions.lstm(c, self.eh(e) + self.hh(h))
        ct = self.calculate_alpha(h, hs_enc)
        t = self.fy(functions.tanh(self.wc(ct) + self.wh(h)))
        return t, c, h

    @staticmethod
    def calculate_alpha(h, hs_enc):
        sum_value = Variable(ARR.zeros((h.shape[0],1),dtype='float32'))
        products = []
        for h_enc in hs_enc:
            inner_product = functions.exp(functions.batch_matmul(h, h_enc, transa=True)).data[:,:,0]
            products.append(inner_product)
            sum_value += inner_product
        ct = Variable(ARR.zeros((h.shape[0], h.shape[1]), dtype='float32'))
...
class Seq2Seq(Chain):
    ...
    def test_decode(self,predict_id):
        ...
        predict_vec, self.c, self.h = self.decoder(word, self.c, self.h, self.h_enc)
        return predict_vec
...
def main():
    ...
    for num, epoch in enumerate(range(EPOCH_NUM)):
        ...
        for i in range(0, len(corpus.rough_posts), BATCH_SIZE):
            ...
            model.reset(batch_size=input_batch.shape[1])
